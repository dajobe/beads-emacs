...
(ert-deftest beads-list-sort-puts-in-progress-above-open-at-equal-priority ()
  (let ((prog '("bve-prog" ["📋" "P1" "PROG" "bve-prog" "Working" "1d ago"]))
        (open '("bve-open" ["📋" "P1" "OPEN" "bve-open" "Waiting" "1d ago"]))
        (blocked '("bve-blkd" ["📋" "P1" "BLKD" "bve-blkd" "Stuck" "1d ago"]))
        (done '("bve-done" ["📋" "P1" "DONE" "bve-done" "Finished" "1d ago"]))
        (urgent '("bve-p0" ["📋" "P0" "OPEN" "bve-p0" "Critical" "1d ago"])))
    (should (beads-list--sort-priority prog open))
    (should-not (beads-list--sort-priority open prog))
    (should (beads-list--sort-priority open blocked))
    (should (beads-list--sort-priority blocked done))
    (should (beads-list--sort-priority urgent prog))
    (should-not (beads-list--sort-priority prog urgent))))
...
(ert-deftest beads-list-filter-keys-match-beads-and-existing-views ()
  ...
  (should (eq (lookup-key beads-list-mode-map (kbd "i"))
